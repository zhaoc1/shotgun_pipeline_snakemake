...
TARGET_TSV = [config["data_dir"]+"/preprocess_summary.tsv"]
print(TARGET_TSV)

TARGET_FPS = TARGET_FPS1 + TARGET_FPS2 + TARGET_TSV
...
rule illqc:
	...
	params:
		...
		illqc_summary = config["data_dir"] + "/summary/summary-illqc_{sample_id}.json"
	shell:
...
rule stitch_reads:
	...
	params:
		...
		stitch_summary = config["data_dir"] + "/summary/summary-stitch_{sample_id}.json",
...
rule decontam_phix_unassembled:
	...
	params:
		...
		decontam_phix_summary_unassembled = config["data_dir"] + "/summary/summary-unassembled-decontam-phix_{sample_id}.json"
	shell:
...
rule decontam_host_unassembled:
	...
	params:
		...
		decontam_host_summary_unassembled = config["data_dir"] + "/summary/summary-unassembled-decontam-host_{sample_id}.json"
	shell:
		"decontaminate.py --forward-reads {input.read1} --reverse-reads {input.read2} "
...
rule decontam_phix_assembled:
	...
	params:
		...
		decontam_phix_summary_assembled = config["data_dir"] + "/summary/summary-assembled-decontam-phix_{sample_id}.json"
	shell:
...
rule decontam_host_assembled:
	...
	params:
		...
		decontam_host_summary_assembled = config["data_dir"] + "/summary/summary-assembled-decontam-host_{sample_id}.json"
	shell:
...
rule generate_report:
	output:
		dnabc = config["data_dir"] + "/preprocess_summary.tsv"
	shell:
		"""
		preprocess_report.py --illqc-dir {config[data_dir]}/summary --decontam-dir {config[data_dir]}/summary \
		--decontam-prefix summary-assembled-decontam-host_ --output-fp {output.dnabc} 
		
		fastqc_report.py --input-dir {config[data_dir]}/illqc_reports --output-dir {config[data_dir]}
		"""
